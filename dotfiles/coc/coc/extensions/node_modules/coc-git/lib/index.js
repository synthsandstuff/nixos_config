...
var Commits = class extends import_coc4.BasicList {
  constructor(nvim, manager) {
    ...
    this.defaultAction = "show";
    this.cachedCommits = /* @__PURE__ */ new Map();
    this.addAction("preview", async (item, context) => {
      let { commit, root } = item.data;
      let lines = [];
      if (commit) {
  ...
  async loadItems(context) {
    ...
    for (let [file, diffs] of diffGroups) {
      for (let diff of diffs) {
        ...
        let location = import_coc8.Location.create(
          uri,
          import_coc8.Range.create(diff.start - 1, 0, diff.end - 1, 0)
        );
        res.push({
...
var DocumentManager = class {
  ...
  async keepCurrent() {
    return this.conflictKeepPart(0 /* Current */);
  }
  async keepIncoming() {
    return this.conflictKeepPart(1 /* Incoming */);
  }
  async keepBoth() {
    return this.conflictKeepPart(2 /* Both */);
  }
...
function addSource(context, resolver) {
  ...
  async function loadGitHubIssues(organizationName, repoName, shouldIncludeOrganizationNameAndRepoNameInAbbr = false) {
    ...
    while (true) {
      ...
      page_idx++;
      try {
        let info = await (0, import_coc15.fetch)(page_uri, { headers });
        if (info.length == 0) {
          break;
        }
        for (let i = 0, len = info.length; i < len; i++) {
...
async function activate(context) {
  ...
  subscriptions.push(import_coc16.commands.registerCommand("git.nextChunk", async () => {
    await manager.nextChunk();
  }));
  subscriptions.push(import_coc16.commands.registerCommand("git.prevChunk", async () => {
    await manager.prevChunk();
  }));
  ...
  subscriptions.push(import_coc16.listManager.registerList(new GStatus(nvim, manager)));
  subscriptions.push(import_coc16.listManager.registerList(new Branches(nvim, manager)));
  subscriptions.push(import_coc16.listManager.registerList(new Commits(nvim, manager)));
  subscriptions.push(import_coc16.listManager.registerList(new Bcommits(nvim, manager)));
    ...
    provideCompletionItems: async (document, position) => {
      ...
      const text = document.getText(
        import_coc16.Range.create(
          import_coc16.Position.create(position.line, 0),
          position
        )
      );
      if (/^[a-z]*$/.test(text)) {
